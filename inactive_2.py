...
output_result = None
LABELS=None
def UpdateMotorAngle(model, layer_name):
    global  cap_image
    ...
    while True:
        if cap_image is None:
            print("image is none")
            continue
        output_result=pedestrian_detection(cap_image, model, layer_name, personidz=LABELS.index("person"))



eyes=RoboticEye2.RoboticEye2(port="/dev/ttyUSB2")
# eyes2 = RoboticEye2.RoboticEye2(port="/dev/ttyUSB3")
# right
eyes.sid_v = 4
eyes.sid_h = 3
# eyes2.sid_h = 1
...
def gaze(status):
    if (abs(status.prev_angle[0] - status.angle[0]) > 30):
...
def main():
    ...
    k = 0
    #i = 1
    t_thread = Thread(target=UpdateMotorAngle,args=(model,layer_name))
    t_thread.start()
    while True:
        ...
        if not grabbed:
            break
        cap_image = imutils.resize(image, width=700)
        image = cap_image
        gaze_direction = (int(image.shape[1] / 2), int(image.shape[0] / 2))

        p = time.time()
        results =output_result #pedestrian_detection(image, model, layer_name, personidz=LABELS.index("person"))
        p2 = time.time() - p
